...
describe('../../src/infra/storage', () => {
    ...
    test('STORAGE_ROOT 未設時 fallback 到 "./storage"', () => {
        delete process.env.STORAGE_ROOT;
        jest.isolateModules(() => {
            const mod = require('../../src/infra/storage');
    ...
    test('storageSubpath("shadowLp", "2026-04.jsonl") 產生正確 path', () => {
        ...
        jest.isolateModules(() => {
            const { storageSubpath } = require('../../src/infra/storage');
    ...
    describe('ensureStorageDir', () => {
        ...
        test('冪等 — 連續呼叫兩次不 throw（shadowLp）', () => {
            jest.isolateModules(() => {
                const { ensureStorageDir } = require('../../src/infra/storage');
        ...
        test('所有 10 個領域皆可建立', () => {
            jest.isolateModules(() => {
                const { ensureStorageDir, STORAGE_PATHS } = require('../../src/infra/storage');
